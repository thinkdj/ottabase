...
**Date:** 2026-02-02 **Status:** Architecture Review **Priority:** Critical - Security & Data Isolation
...
**Current State:** These two dimensions exist but are **incompletely integrated**, creating security gaps and
...
**File:** `packages/rbac/src/cache.ts` **Impact:** Cross-tenant data leakage
...
**Root Cause:** organizationId is **optional** in cache key generation. If not passed, cache keys collide across
organizations.
...
**File:** `packages/rbac/src/cache.ts` **Impact:** O(n) cache clearing on role changes
...
**File:** `packages/ottaorm/src/context/index.ts` **Impact:** No database connection per app
