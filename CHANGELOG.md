...
- **Paid (premium) package framework — `@ottabase/premium`.** Sell add-ons for an Ottabase app, or install someone
  else's, without either side writing integration code. One `definePremiumPackage()` manifest carries everything a paid
  package contributes — Drizzle tables, migrations, OttaORM models, API routes, admin nav, entitlements and lifecycle
  hooks — and installing one is a single line in the new `apps/*/ottabase/config.premium.ts`. Paid packages deliberately
  bypass the free-package wiring (`config.migrations.ts` PACKAGE_REGISTRY, `config.routes.ts`, `ottabase.config.ts`,
  `db-utils.ts`): everything is derived from the manifest list, so there is exactly one place that decides whether a
  paid package exists.
    ...
    - **An app with no paid packages is unaffected**: no routes, no middleware, no KV reads, no nav entries, no tables.
      Full guide in `docs/PREMIUM_PACKAGES.md`.
