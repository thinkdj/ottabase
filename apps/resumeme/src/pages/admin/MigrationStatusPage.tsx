// ---------------------------------------------------------------------------
// MigrationStatusPage — Database migration status & init tool for admins.
// Runs /api/ottaorm/init on mount, shows table status, columns added,
// custom migrations, and errors.
// ---------------------------------------------------------------------------
...
export function MigrationStatusPage() {
    ...
    /** Convert camelCase table variable name → snake_case entity name */
    const convertToSnakeCase = (str: string): string => {
        const withoutSuffix = str.replace(/Table$/, '');
    ...
    const getTableInfo = (tableVarName: string) => {
        if (!modelsMetadata?.models) {
            return { actualName: tableVarName, category: 'Unknown' as const, packageName: 'unknown' };
        }
        ...
        return { actualName: model.tableName, category, packageName: model.packageName };
    };

    /** Build table status from init result */
                ...
                <div className="grid gap-6">
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            {/* Table listing with category filters */}
                                            ...
                                            {(() => {
                                                ...
                                                const sortedTables = [...initResult.details.tablesDetected]
                                                    ...
                                                    .sort((a, b) => {
                                                        ...
                                                        if (orderA === orderB) {
                                                            return getTableInfo(a).actualName.localeCompare(
                                                                getTableInfo(b).actualName,
                                                            );
                                                        }
                                                ...
                                                return sortedTables.map((tableVarName) => {
                                ...
                                <div className="border rounded-lg overflow-hidden">
                                    ...
                                    <div className="p-4">
                                        <ul className="list-disc list-inside text-xs space-y-1">
                                            {initResult.details.columnsAdded.map((col) => (
                                    ...
                                    <div className="p-4 space-y-3">
                                        {initResult.details.customMigrationsRun.length > 0 && (
                                            <div>
                                                ...
                                                <ul className="list-disc list-inside text-xs space-y-1 ml-2">
                                                    {initResult.details.customMigrationsRun.map((mig) => (
                                        ...
                                        {initResult.details.customMigrationsSkipped.length > 0 && (
                                            <div>
                                                ...
                                                <ul className="list-disc list-inside text-xs space-y-1 ml-2">
                                                    {initResult.details.customMigrationsSkipped.map((mig) => (
                                                        <li key={mig} className="font-mono text-muted-foreground">
                                                            {mig}
                                                        </li>
                                                    ))}
                            ...
                            {initResult.details.errors.length > 0 && (
                                <div className="border border-destructive/30 rounded-md p-4 bg-destructive/5">
                                    ...
                                    <ul className="list-disc list-inside text-sm text-destructive space-y-1">
                                        {initResult.details.errors.map((err) => (
                                            <li key={err}>{err}</li>
                                        ))}
                                    </ul>
                                </div>
