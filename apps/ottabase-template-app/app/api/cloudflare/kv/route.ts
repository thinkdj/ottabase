...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!env.OBCF_KV) {
            return NextResponse.json({ error: 'KV namespace binding not configured' }, { status: 500 });
        }
        ...
        if (!result.success) {
            return NextResponse.json({ error: 'Failed to get value', details: result.error.message }, { status: 500 });
        }
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!result.success) {
            return NextResponse.json({ error: 'Failed to set value', details: result.error.message }, { status: 500 });
        }
