...
//
// PREMIUM PACKAGES need NONE of the above: their tables and migrations are read
...
export function getEnabledPackageTables(env?: Record<string, unknown>) {
    ...
    // Premium Packages (from ottabase/config.premium.ts). Their tables are created as soon as
...
export function getEnabledPackageMigrations(env?: Record<string, unknown>): Migration[] {
    ...
    // Premium Packages (from ottabase/config.premium.ts)
