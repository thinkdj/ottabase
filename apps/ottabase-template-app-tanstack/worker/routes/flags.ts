...
export async function handleFlagsEvaluate(context: FlagsRouteContext): Promise<Response> {
    ...
    // Build evaluation context from the current session
    const session = await getSession(request, env as any, getAuthOptions(env));
    const userId = session?.user?.id ?? undefined;
    const organizationId = request.headers.get('x-organization-id') || session?.user?.organizationId || undefined;
...
export async function handleFlagById(
    ...
    if (body.name !== undefined) flag.set('name', body.name);
    if (body.description !== undefined) flag.set('description', body.description);
    if (body.enabled !== undefined) flag.set('enabled', body.enabled);
    if (body.rules !== undefined) flag.set('rules', body.rules);
