  ...
  return (
    <div className={`rounded-lg border p-4 ${colorClasses} ${className}`}>
      <div className="space-y-2">
        ...
        {apiError?.details && (
          <p className={`text-sm opacity-80 ${textClasses}`}>{apiError.details}</p>
        )}

        {apiError?.hint && (
          <p className={`inline-block rounded px-2 py-1 text-sm font-mono ${isWarning ? "bg-amber-500/10" : "bg-destructive/10"} ${textClasses}`}>
            {apiError.hint}
          </p>
        )}
