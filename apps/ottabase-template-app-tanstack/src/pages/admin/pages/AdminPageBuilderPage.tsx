...
export function AdminPageBuilderPage() {
    ...
    const sections = useMemo(() => {
        const rows = (sectionList.data ?? []) as any[];
        return [...rows].sort((a, b) => Number(a.sortOrder) - Number(b.sortOrder));
    }, [sectionList.data]);

    const selected = sections.find((section) => section.id === selectedId) ?? null;
    const selectedFeatures = useMemo(() => {
        const rows = (featureList.data ?? []) as any[];
        return [...rows].sort((a, b) => Number(a.sortOrder) - Number(b.sortOrder));
    }, [featureList.data]);
    const selectedActions = useMemo(() => {
        const rows = (actionList.data ?? []) as any[];
        return [...rows].sort((a, b) => Number(a.sortOrder) - Number(b.sortOrder));
    }, [actionList.data]);
    ...
    }, [(pageQuery.data as any)?.id, (pageQuery.data as any)?.updatedAt]);
                ...
                <div className="flex items-center gap-2">
                    <Button
                        ...
                        onClick={async () => {
                            ...
                            await updatePage.mutateAsync({ id: page.id, data: { status: nextStatus } as any });
                    ...
                    <Button
                        onClick={async () => {
                            ...
                            await updatePage.mutateAsync({
                                id: page.id,
                                data: { title: page.title, slug: page.slug } as any,
                            });
                                            ...
                                            <Input
                                                ...
                                                onBlur={async () => {
                                                    await updateFeature.mutateAsync({
                                                        id: feature.id,
                                                        data: { title: feature.title } as any,
                                                    });
                                                }}
                                            />
                                            ...
                                            <Input
                                                ...
                                                onBlur={async () => {
                                                    await updateAction.mutateAsync({
                                                        id: action.id,
                                                        data: { label: action.label, href: action.href } as any,
                                                    });
                                                }}
                                            />
                                            <Button
