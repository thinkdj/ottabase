...
export function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (!hasAnySocialLogin && !hasMagicLink) {
      newWarnings.push(
        "No OAuth providers or Magic Link configured. Only credentials login available.",
      );
    }
        ...
        <Card className="mt-4 border-green-500/50 bg-green-50 dark:bg-green-950/20">
          <CardHeader>
            ...
            <CardDescription className="text-xs">
              Powered by Auth.js with Cloudflare D1
            </CardDescription>
          </CardHeader>
          <CardContent className="text-xs space-y-2">
            <p>
              <strong>Credentials:</strong> Email/password authentication with
              secure hashing
            </p>
            <p>
              <strong>Social Login:</strong> OAuth 2.0 providers (Google,
              GitHub, Discord, etc.)
            </p>
            <p>
