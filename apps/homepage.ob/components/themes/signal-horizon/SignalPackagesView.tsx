...
export function SignalPackagesView() {
    ...
    return (
        <div className="hp-container">
            <div className="hp-filter" role="toolbar" aria-label="Filter packages by category">
                {FILTER_ORDER.map((f) => (
                    <button
                        key={f.id}
                        type="button"
                        className={`hp-filter-btn${filter === f.id ? ' is-active' : ''}`}
                        data-cat={f.id}
                    ...
                    >
                        ...
                        {f.label} <span style={{ color: 'var(--hp-dim)', marginLeft: 4 }}>{f.count}</span>
                    </button>
                ))}
            </div>
                ...
                {PACKAGE_SECTIONS.map((section) => {
                    ...
                    return (
                        <div key={section.category} data-cat={section.category}>
                            <h2 className="hp-pkg-section-title hp-reveal">
                                <span
                            ...
                            <div className="hp-pkg-grid hp-reveal">
