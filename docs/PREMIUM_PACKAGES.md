...
**An app with no paid packages is unaffected.** Leave `PREMIUM_PACKAGES` empty and nothing mounts: no routes, no KV
...
- **Pass the app's API client** to `request`. It attaches `X-Org-Id` and `X-App-Id`, which select the tenant the server
  resolves against.
- Add the package to the app's Tailwind `content` array. Omitting it fails **silently** — components render unstyled.
