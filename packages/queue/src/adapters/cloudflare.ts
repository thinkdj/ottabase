...
import type { QueueAdapter, AdapterResult, SendOptions, QueueMessage, QueueMessageBatch } from './types';
...
export class CloudflareAdapter implements QueueAdapter {
     ...
     */
    async send(job: QueuedJob, options?: SendOptions): Promise<AdapterResult<void>> {
     ...
     */
    async sendBatch(messages: Array<{ body: QueuedJob; options?: SendOptions }>): Promise<AdapterResult<void>> {
        const cfMessages = messages.map((m) => ({
            body: m.body,
            options: m.options?.delaySeconds ? { delaySeconds: m.options.delaySeconds } : undefined,
        }));
 ...
 * ```
 */
export function toAdapterBatch<T>(cfBatch: import('@cloudflare/workers-types').MessageBatch<T>): QueueMessageBatch<T> {
    return {
        queue: cfBatch.queue,
