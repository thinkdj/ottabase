...
const entry = Object.fromEntries(
    componentEntries.map((filePath) => {
        // Preserve internal lib folder structure so relative imports like
        // `./lib/utils` resolve correctly from dist outputs.
        if (filePath.startsWith('src/lib/')) {
            const outPath = filePath.replace(/^src\//u, '').replace(/\.(tsx|ts)$/u, '');
