...
describe('client-api changePassword', () => {
    ...
    it('returns null when the session response has no user', async () => {
        vi.stubGlobal(
            'fetch',
            vi.fn(async () => ({ ok: true, json: async () => null })) as unknown as typeof fetch,
        );
    ...
    it('surfaces the server error message on failure', async () => {
        vi.stubGlobal(
            ...
            vi
                .fn()
                .mockResolvedValueOnce({ ok: true, json: async () => ({ csrfToken: 'csrf-token-value' }) })
                .mockResolvedValueOnce({ ok: false, json: async () => ({ error: 'Invalid email or password' }) }) as unknown as typeof fetch,
        );
