...
export const authenticatorsTable = sqliteTable(
    ...
    (t) => ({
        // Listing a user's registered passkeys filters by userId; credentialId is UNIQUE-indexed
        // for the per-authentication lookup.
        userIdx: index('authenticators_user_idx').on(t.userId),
    }),
);
