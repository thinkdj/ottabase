...
export const mediaTable = sqliteTable(
    ...
    (t) => ({
        // Media is RLS-scoped by organization/user/app and the library UI orders by createdAt.
        // Composite (organizationId, createdAt) serves the tenant-scoped, time-ordered listing.
        orgCreatedIdx: index('media_org_created_idx').on(t.organizationId, t.createdAt),
        userIdx: index('media_user_idx').on(t.userId),
        appIdx: index('media_app_idx').on(t.appId),
    }),
);
