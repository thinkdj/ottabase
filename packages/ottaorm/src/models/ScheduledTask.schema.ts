...
export const scheduledTasksTable = sqliteTable(
    ...
    (t) => ({
        // The DB-backed cron scheduler polls due tasks: WHERE is_active = 1 AND next_run_at <= now.
        // A composite (is_active, next_run_at) index turns that poll into an index range scan.
        dueIdx: index('scheduled_tasks_active_next_idx').on(t.isActive, t.nextRunAt),
    }),
);
