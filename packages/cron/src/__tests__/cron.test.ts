...
describe("CronHandler", () => {
  ...
  describe("on()", () => {
    it("should register a handler for a cron expression", () => {
      const handler = createCronHandler<TestEnv>().on(
        "0 0 * * *",
        async () => {}
      );
  ...
  describe("handler", () => {
    ...
    it("should warn when no handler is registered for cron expression", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "[cron] No handler registered for: 0 0 * * *"
      );
  ...
  describe("hooks", () => {
    it("should call onBeforeJob before handler", async () => {
      ...
      const cron = createCronHandler<TestEnv>({ onBeforeJob }).on(
        "0 0 * * *",
        handler
      );
    ...
    it("should call onAfterJob after handler", async () => {
      ...
      const cron = createCronHandler<TestEnv>({ onAfterJob }).on(
        "0 0 * * *",
        handler
      );
    ...
    it("should call onError when handler throws", async () => {
      ...
      const cron = createCronHandler<TestEnv>({ onError }).on(
        "0 0 * * *",
        handler
      );
    ...
    it("should rethrow error when no onError hook provided", async () => {
      ...
      await expect(
        cron.handler(createMockEvent("0 0 * * *"), mockEnv, mockCtx)
