...
export class R2Client {
    ...
    constructor(config: R2Config) {
        if (!config.bucket) {
            throw new CloudflareError('R2 bucket binding is required', 'R2_MISSING_BINDING');
        }
     ...
     */
    async get(key: string, options?: R2GetOptions): Promise<Result<R2ObjectBody | null, Error>> {
        try {
     ...
     */
    async createMultipartUpload(key: string, options?: R2PutOptions): Promise<Result<R2MultipartUpload, Error>> {
        try {
