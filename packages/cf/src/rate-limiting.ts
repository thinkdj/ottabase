...
export class RateLimitingClient {
    ...
    constructor(config: RateLimitingConfig) {
        if (!config.rateLimiter) {
            throw new CloudflareError('RateLimiter binding is required', 'RATE_LIMITING_MISSING_BINDING');
        }
    ...
    async check(options: RateLimitOptions): Promise<Result<void, Error>> {
        ...
        if (!result.data.success) {
            return {
                success: false,
                error: new CloudflareError('Rate limit exceeded', 'RATE_LIMIT_EXCEEDED', {
