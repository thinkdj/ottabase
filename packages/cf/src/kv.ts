...
export class KVClient {
    ...
    constructor(config: KVConfig) {
        if (!config.namespace) {
            throw new CloudflareError('KV namespace binding is required', 'KV_MISSING_BINDING');
        }
     ...
     */
    async get<T = unknown>(key: string, options?: KVGetOptions): Promise<Result<T | null, Error>> {
        try {
     ...
     * Put a JSON value
     */
    async putJSON<T = unknown>(key: string, value: T, options?: KVPutOptions): Promise<Result<void, Error>> {
        try {
