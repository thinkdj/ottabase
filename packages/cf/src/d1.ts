...
export class D1Client {
    ...
    constructor(config: D1Config) {
        if (!config.database) {
            throw new CloudflareError('D1 database binding is required', 'D1_MISSING_BINDING');
        }
     ...
     */
    async query<T = unknown>(sql: string, params?: unknown[], options?: D1QueryOptions): Promise<Result<T[], Error>> {
        try {
            const stmt = params ? this.db.prepare(sql).bind(...params) : this.db.prepare(sql);
            const result = await stmt.all<T>();

            if (!result.success) {
                throw new CloudflareError(result.error || 'Query failed', 'D1_QUERY_ERROR');
            }
     ...
     */
    async execute(sql: string, params?: unknown[], options?: D1QueryOptions): Promise<Result<D1Result, Error>> {
        try {
            const stmt = params ? this.db.prepare(sql).bind(...params) : this.db.prepare(sql);
            const result = await stmt.run();

            if (!result.success) {
                throw new CloudflareError(result.error || 'Execute failed', 'D1_EXECUTE_ERROR');
            }
