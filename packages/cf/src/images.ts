...
export class ImagesClient {
    ...
    constructor(config: ImagesConfig) {
        if (!config.accountId || !config.apiToken) {
            throw new CloudflareError('Images accountId and apiToken are required', 'IMAGES_MISSING_CONFIG');
        }
     ...
     * Upload an image
     */
    async upload(image: File | Blob | ArrayBuffer, options?: ImageUploadOptions): Promise<Result<ImageDetails, Error>> {
        try {
            ...
            if (!response.ok) {
                throw new CloudflareError(`Upload failed: ${response.statusText}`, 'IMAGES_UPLOAD_ERROR', {
    ...
    async get(imageId: string): Promise<Result<ImageDetails, Error>> {
        try {
            ...
            if (!response.ok) {
                throw new CloudflareError(`Get image failed: ${response.statusText}`, 'IMAGES_GET_ERROR', {
                    status: response.status,
                });
            }
    ...
    async delete(imageId: string): Promise<Result<void, Error>> {
        try {
            ...
            if (!response.ok) {
                throw new CloudflareError(`Delete failed: ${response.statusText}`, 'IMAGES_DELETE_ERROR', {
                    status: response.status,
                });
            }
    ...
    async list(
        page: number = 1,
        perPage: number = 50,
    ): Promise<Result<{ images: ImageDetails[]; total: number }, Error>> {
        try {
            ...
            if (!response.ok) {
                throw new CloudflareError(`List failed: ${response.statusText}`, 'IMAGES_LIST_ERROR', {
                    status: response.status,
                });
            }
