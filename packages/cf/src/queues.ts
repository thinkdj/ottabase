...
export class QueuesClient<T = unknown> {
    ...
    constructor(config: QueuesConfig) {
        if (!config.queue) {
            throw new CloudflareError('Queue binding is required', 'QUEUE_MISSING_BINDING');
        }
     ...
     */
    async sendBatch(messages: Array<{ body: T; options?: QueueSendOptions }>): Promise<Result<void, Error>> {
        try {
 ...
 */
export type QueueHandler<T = unknown> = (batch: MessageBatch<T>, env: unknown) => Promise<void> | void;
