...
- **Global Theming**: All components use CSS custom properties (`--primary`, `--muted`, `--border`, etc.) and adapt to
...
The `CTA` component renders a theme-aware call-to-action button using CSS custom properties rather than fixed Tailwind
colour classes, so it respects whatever global theme is active.
