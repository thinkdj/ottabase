...
export function validateFiles(
  ...
  if (config.maxFiles && files.length > config.maxFiles) {
    errors.push(
      `Maximum ${config.maxFiles} file${config.maxFiles > 1 ? 's' : ''} allowed`
    );
  }
  ...
  files.forEach((file, index) => {
    ...
    if (config.maxFileSize && !validateFileSize(file, config.maxFileSize)) {
      errors.push(
        `File "${file.name}" exceeds maximum size of ${formatFileSize(config.maxFileSize)}`
      );
    }
    ...
    if (
      ...
      !validateFileType(file, config.acceptedFileTypes)
    ) {
      errors.push(
        `File "${file.name}" has invalid type. Accepted types: ${config.acceptedFileTypes.join(', ')}`
      );
    }
  });
