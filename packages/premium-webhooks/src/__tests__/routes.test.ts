...
type TestCaller = { userId: string | null; organizationId: string | null; appId: string | null; canManage: boolean };

let caller: TestCaller | null;

function build(options?: { endpointQuota?: WebhookEndpointQuota<Env> }): {
    app: Router<Env>;
    ...
    const pkg = createWebhooksPackage<Env>({
        ...
        events: ['*', 'todo.created'],
        endpointQuota: options?.endpointQuota,
    });
...
describe('distributed endpoint quota seam', () => {
    it('uses the host coordinator to refuse a final slot held by another isolate', async () => {
        const endpointQuota = {
            reserve: vi.fn(async () => null),
            synchronize: vi.fn(async () => undefined),
        };
        const { app } = build({ endpointQuota });

        const response = await app.handle(post('/', { url: 'https://example.com/h' }), FREE);

        expect(response?.status).toBe(402);
        expect(endpointQuota.reserve).toHaveBeenCalledWith(
            FREE,
            expect.objectContaining({ organizationId: 'org-1', appId: 'otta-web' }),
            { current: 0, limit: 1 },
        );
        expect(tables.endpoints).toHaveLength(0);
    });

    it('commits a host reservation after creating the endpoint', async () => {
        const commit = vi.fn(async () => undefined);
        const release = vi.fn(async () => undefined);
        const { app } = build({
            endpointQuota: { reserve: async () => ({ commit, release }), synchronize: async () => undefined },
        });

        expect((await app.handle(post('/', { url: 'https://example.com/h' }), FREE))?.status).toBe(201);
        expect(commit).toHaveBeenCalledTimes(1);
        expect(release).not.toHaveBeenCalled();
    });

    it('returns the created endpoint when only reservation cleanup fails', async () => {
        const release = vi.fn(async () => undefined);
        const { app } = build({
            endpointQuota: {
                reserve: async () => ({
                    commit: async () => Promise.reject(new Error('coordinator unavailable')),
                    release,
                }),
        ...
        expect((await app.handle(post('/', { url: 'https://example.com/h' }), FREE))?.status).toBe(201);
        expect(release).not.toHaveBeenCalled();
        expect(tables.endpoints).toHaveLength(1);
    });
});
