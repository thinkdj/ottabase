...
export function ZoomableImage({ src, alt, className, mode = 'lightbox', zoomStart = 'single' }: ZoomableImageProps) {
    ...
    return (
        ...
        >
            ...
            {isZoomed && (
                <span className="pointer-events-none absolute bottom-2 right-2 select-none rounded-full bg-white/15 px-2 py-0.5 text-[0.6875rem] font-medium tabular-nums text-white ring-1 ring-white/30 backdrop-blur-md">
                    {zoom.toFixed(1)}x
                </span>
            )}
        </div>
