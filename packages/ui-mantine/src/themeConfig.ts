    ...
    Object.keys(override).forEach((componentName) => {
      if (!merged[componentName]) {
        ...
        merged[componentName] = {
          ...
          styles:
            override[componentName]?.styles || merged[componentName]?.styles,
        };
      }
    });
 ...
 */
export function validateMantineThemeConfig(
  config: MantineThemeConfig,
): string[] {
  const errors: string[] = [];
  ...
  if (
    config.primaryShade !== undefined &&
    (config.primaryShade < 0 || config.primaryShade > 9)
  ) {
  ...
  if (config.colors) {
    Object.entries(config.colors).forEach(([colorName, colorArray]) => {
      if (!Array.isArray(colorArray) || colorArray.length !== 10) {
        errors.push(
          `Color '${colorName}' must be an array of exactly 10 hex colors`,
        );
      } else {
        colorArray.forEach((color, index) => {
          if (typeof color !== "string" || !color.match(/^#[0-9a-fA-F]{6}$/)) {
            errors.push(
              `Color '${colorName}[${index}]' must be a valid hex color (e.g., #ffffff)`,
            );
          }
        });
      }
