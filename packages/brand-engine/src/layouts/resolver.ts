...
export function resolveRouteForPath(
    pathname: string,
    routeMappings: Array<{ pathPattern: string; layoutTemplateId: string; brandKitId: string; priority?: number; tokenOverridesJson?: string | null }>,
): RouteMatchResult | null {
    const sorted = [...routeMappings].sort((a, b) => (b.priority ?? 0) - (a.priority ?? 0));
    for (const m of sorted) {
        const re = pathPatternToRegex(m.pathPattern);
        if (re.test(pathname)) return { layoutTemplateId: m.layoutTemplateId, brandKitId: m.brandKitId, tokenOverridesJson: m.tokenOverridesJson };
    }
    return null;
}
