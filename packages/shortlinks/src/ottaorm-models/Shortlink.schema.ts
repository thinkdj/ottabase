 ...
 */
export const shortlinksTable = sqliteTable(
    'shortlinks',
    {
        id: text('id')
            .primaryKey()
    ...
    (t) => ({
        // Shortlinks are app-scoped by RLS when listed in the dashboard (filtered by appId).
        // shortCode is already UNIQUE-indexed for the hot per-hit redirect resolution.
        appIdx: index('shortlinks_app_idx').on(t.appId),
    }),
);
