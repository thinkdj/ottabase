...
export function Logo({
    ...
    const renderDarkModeToggle = () => darkModeSwitcher && <DarkModeToggle type="button" />;

    const LogoContent = () => (
        <>
            {appLogoUrl && (
                <img src={appLogoUrl} alt={`${appNameTitle}`} height={size} className="rounded object-contain" />
            )}
            {appNameTitle && <h4 className="text-lg font-semibold">{appNameTitle}</h4>}
        </>
    );

    return (
        <div className={`flex flex-row flex-nowrap items-center justify-between gap-4 ${minWidthClass}`}>
