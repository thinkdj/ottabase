...
import { createKvStateStore, createMemoryStateStore } from '../state-store';
...
describe('an app with no Premium Packages', () => {
...
describe('license sources', () => {
    it('treats a structurally corrupt KV record as absent instead of passing an object to license verification', async () => {
        const kv = {
            get: vi.fn(async () => JSON.stringify({ version: '1', installedAt: 1, updatedAt: 1, license: {} })),
            put: vi.fn(),
            delete: vi.fn(),
            list: vi.fn(),
        };
        const registry = createPremiumRegistry({
            ...
            getStore: () => createKvStateStore(kv),
            cacheTtlMs: 0,
        });

        await expect(registry.status({}, 'webhooks')).resolves.toMatchObject({ state: 'unlicensed' });
    });
    ...
    it('does not replace a working stored license with an invalid activation attempt', async () => {
        const { registry, store } = makeRegistry();
        await registry.activate({}, 'webhooks', license);

        const rejected = await registry.activate({}, 'webhooks', 'obp1.not-a-license.signature');

        expect(rejected).toMatchObject({ state: 'invalid', reason: 'LICENSE_MALFORMED' });
        expect((await store.get('webhooks'))?.license).toBe(license);
        expect(await registry.status({}, 'webhooks')).toMatchObject({ state: 'active', licenseSource: 'store' });
    });
...
describe('lifecycle', () => {
    ...
    it('invalidates the environment-scoped resolution cache after uninstall', async () => {
        const store = createMemoryStateStore();
        const registry = createPremiumRegistry({
            packages: [makePackage()],
            getStore: () => store,
        ...
        const env = {};
        await registry.activate(env, 'webhooks', license);
        expect((await registry.status(env, 'webhooks'))?.state).toBe('active');

        await registry.uninstall(env, 'webhooks');

        expect((await registry.status(env, 'webhooks'))?.state).toBe('unlicensed');
    });
});
