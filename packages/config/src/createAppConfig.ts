...
export function createAppConfig(options: ConfigOptions = {}): AppConfig {
  ...
  const meta: AppMeta = {
    ...
    title: getEnv(
      "APP_TITLE",
      defaults.meta?.title ?? getEnv("APP_NAME", appName),
    ),
    author: getEnv("APP_AUTHOR", defaults.meta?.author ?? "@thinkdj"),
    description: getEnv(
      "APP_DESCRIPTION",
      defaults.meta?.description ??
        `A modern web application built with ${appName}`,
    ),
    keywords: getEnv(
      "APP_KEYWORDS",
      defaults.meta?.keywords ??
        "React, Next.js, TypeScript, Mantine, Tailwind",
    ),
    robots: getEnv("APP_ROBOTS", defaults.meta?.robots ?? "index, follow"),
    copyrightText: getEnv(
      "APP_COPYRIGHT_TEXT",
      defaults.meta?.copyrightText ??
        `© 2020-${new Date().getFullYear()} @thinkdj`,
    ),
  ...
  const config: AppConfig = {
    ...
    uiFramework: getEnv(
      "UI_FRAMEWORK",
      defaults.uiFramework ?? "mantine",
    ) as SupportedUIFramework,

    ui: {
      preventFOUC: getBoolEnv(
        "PREVENT_FOUC",
        defaults.ui?.preventFOUC ?? false,
      ),
      preventFOUCInsideIframe: getBoolEnv(
        "PREVENT_FOUC_INSIDE_IFRAME",
        defaults.ui?.preventFOUCInsideIframe ?? false,
      ),
      debounceMs: getNumberEnv(
        "UI_DEBOUNCE_MS",
        defaults.ui?.debounceMs ?? 500,
      ),
      layout: {
        minWidth: getNumberEnv(
          "UI_LAYOUT_MIN_WIDTH",
          defaults.ui?.layout?.minWidth ?? 320,
        ),
        maxWidth: getNumberEnv(
          "UI_LAYOUT_MAX_WIDTH",
          defaults.ui?.layout?.maxWidth ?? 1280,
        ),
      },
      enforceGoogleFonts: getBoolEnv(
        "ENFORCE_GOOGLE_FONTS",
        defaults.ui?.enforceGoogleFonts ?? true,
      ),
    },
    ...
    theme: {
      colorDefault: getEnv(
        "THEME_COLOR_DEFAULT",
        defaults.theme?.colorDefault ?? "tremorBlue",
      ),
    ...
    storage: {
      prefix: getEnv(
        "STORAGE_PREFIX",
        defaults.storage?.prefix ?? appName.toLowerCase().replace(/\s+/g, "-"),
      ),
    },

    api: {
      serverErrorHttpCode: getNumberEnv(
        "SERVER_ERROR_HTTP_CODE",
        defaults.api?.serverErrorHttpCode ?? 500,
      ),
    },
    ...
    features: {
      ...
      spotlight: {
        enabled: getBoolEnv(
          "SPOTLIGHT_ENABLED",
          defaults.features?.spotlight?.enabled ?? true,
        ),
        shortcuts: defaults.features?.spotlight?.shortcuts ?? [
          "mod + K",
          "mod + ?",
          "?",
          "mod + /",
      ...
      crudHub: {
        apiBaseUrl: getEnv(
          "CRUDHUB_API_BASE_URL",
          defaults.features?.crudHub?.apiBaseUrl ?? "/api/crudhub",
        ),
        urlBase: getEnv(
          "CRUDHUB_URL_BASE",
          defaults.features?.crudHub?.urlBase ?? "crudhub",
        ),
      ...
      auth: {
        signInUrl: getEnv(
          "AUTH_SIGN_IN_URL",
          defaults.features?.auth?.signInUrl ?? "/api/auth/signin",
        ),
        signOutUrl: getEnv(
          "AUTH_SIGN_OUT_URL",
          defaults.features?.auth?.signOutUrl ?? "/api/auth/signout",
        ),
        preLaunchOptIn: getBoolEnv(
          "PRE_LAUNCH_OPT_IN",
          defaults.features?.auth?.preLaunchOptIn ?? false,
        ),
      },
      pagination: {
        ...
        maxPageSize: getNumberEnv(
          "PAGE_SIZE_MAX",
          defaults.features?.pagination?.maxPageSize ?? 100,
        ),
        sizeOptions: defaults.features?.pagination?.sizeOptions ?? [
          5, 10, 20, 50, 100,
        ],
      },
      referrals: {
    ...
    model: {
      defaultRelKey: getEnv(
        "MODEL_DEFAULT_REL_KEY",
        defaults.model?.defaultRelKey ?? "defaults",
      ),
    },
  };
